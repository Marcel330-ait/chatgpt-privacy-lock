...
- The extension stores display preferences such as language and selected protected sidebar areas in `chrome.storage.local`.
...
- 扩展会把语言和所选保护区域等显示偏好存储在 `chrome.storage.local`。
